...
- Auth starts from a sanitized UI cache cookie when present, so header/nav state can render before a server round trip. If that readable auth-state cookie is absent, `useAuth()` must start as anonymous and must not call `/api/auth/session` or `/api/auth/custom-token` just to discover that no session exists. The readable UI cache must never contain ID tokens, refresh tokens, service credentials, private profile data, or bearer JWTs.
- `src/firebase/config.js` must be lazy. Public page imports may read its inert config helpers, but they must not initialize Firebase Auth, Analytics, Functions, or Firebase AI until a cached auth cookie exists, the user starts sign-in, or a signed-in product feature explicitly calls a lazy getter such as `getFirebaseAuth()`, `getFirebaseFunctions()`, or `getFirebaseAI()`.
- Do not initialize Firebase Analytics or Firebase AI on public first paint. Firebase's web config/API key values are project identifiers rather than secrets, but unnecessary `firebase.googleapis.com/.../webConfig` calls on anonymous marketing pages increase observable surface area and should be avoided.
...
- `VITE_AUTH_SESSION_VALIDATE_INTERVAL_MINUTES` controls how often an existing UI cache revalidates with `/api/auth/session` on startup. The default is `15`, which avoids a blocking auth API call on every page while keeping role/access changes bounded by a short refresh window.
