...
- Home hero background video uses `src/trading/components/HeroBackgroundVideo.jsx`. It must use a native `<video>` element without a `controls` attribute; do not use YouTube/Vimeo/third-party iframes for hero background video because their player overlays can reappear after tab visibility changes. Serve hero media through `https://api.newleafsystem.com/api/v1/public/media/home/` with HLS (`.m3u8`) plus MP4 fallback so the browser can stream progressively and adapt where supported.
