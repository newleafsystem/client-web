...
- Home hero background video uses `src/trading/components/HeroBackgroundVideo.jsx`. Keep it as a lazy-mounted, non-interactive background player: no visible iframe controls, one viewport-specific source at a time, `youtube-nocookie.com`, and HD/adaptive streaming hints. Do not embed both desktop and mobile hero iframes at first paint.
